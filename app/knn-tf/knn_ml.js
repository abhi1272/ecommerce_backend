...
const callKnn = (predictPoint) => {
  let { features, labels, testFeatures, testLabels } = loadCSV(
    "./public/pune_places.csv",
    {
      shuffle: true,
      splitTest: 500,
      dataColumns: ["lat", "lon"],
  ...
  const result = knn(features, labels, predictPoint, 10);
  console.log(result);
  return result
};

//  let result = callKnn(tf.tensor([18.4355242, 73.8929614]))

//  const time = moment().add(+result, 'seconds').calendar()
// console.log(result , moment().calendar(), time)
module.exports = {
  callKnn
}